 ...
 *
 * Зачем: при 500k уведомлений/сутки (~250 МБ/сутки, см. раздел 3.6 плана) хранить данные вечно
...
export class RetentionService implements OnApplicationBootstrap {
   ...
   *
   * Зачем: закрывает раздел 3.6 плана — хранилище не растёт бесконечно без ручных операций.
   ...
   *
   * Зачем: имя таблицы нельзя параметризовать в DDL — единственная защита от использования
   * непроверенных имён — строгая проверка перед подстановкой (см. правило 9 и раздел 6.9 плана).
   *
   * @param trx - активная транзакция
