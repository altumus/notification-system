...
| `JWT_SECRET`              | `openssl rand -base64 48` — обязателен, 32+ символа   |
| `NODE_ENV`                | `production`                                          |
| `AUTH_DEV_TOKENS_ENABLED` | `true` только для публичного демо-стенда (см. ниже)   |
| `CORS_ORIGINS`            | `*` или ваш домен                                     |
| `HTTP_RATE_LIMIT_ENABLED` | `true` — лимит частоты запросов на IP (см. ниже)      |

### Что проверяется при старте в production

При `NODE_ENV=production` приложение **не поднимется**, если `JWT_SECRET` не задан (подставился
бы дефолт из репозитория) или короче 32 символов. Это защита от «забыли переменную» — с
публично известным секретом токен может подделать любой, кто видел репозиторий.

Дополнительно в лог старта уходят предупреждения (старт не блокируют):

- `AUTH_DEV_TOKENS_ENABLED=true` — `POST /auth/dev-token` выдаёт токен на **любой** `userId` и
  роль `service`, то есть авторизация на стенде фактически отключена. Для тестового стенда это
  осознанный компромисс: демо-страница и `pnpm smoke` работают без ручной настройки токенов.
  На реальном проде — `false`.
- `CORS_ORIGINS=*` — API доступен с любого origin.

### Лимит частоты запросов на публичном стенде

`HTTP_RATE_LIMIT` (по умолчанию 300 запросов в минуту с одного IP на один эндпоинт) защищает
стенд от флуда до валидации и до запроса к БД; `/auth/dev-token` ограничен жёстче — 20/мин;
`/health/*` из лимита исключён, иначе пробы Railway уронили бы деплой сами. Подробнее —
[ADR-0008](./adr/0008-http-rate-limit.md).

Счётчики хранятся в памяти инстанса: при масштабировании на N реплик эффективный лимит
становится N × значения. Выключать (`HTTP_RATE_LIMIT_ENABLED=false`) стоит только для прогонов
k6 — нагрузка идёт с одного IP (см. [performance.md](./performance.md)).
